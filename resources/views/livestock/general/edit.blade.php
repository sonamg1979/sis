...
<style>
    ...
    <div class="card card-body" style="max-width: 40rem;">
        ...
        {!! Form::open(['action' => ['LivestockGeneralController@update',$info->id],'method' => 'POST','enctype'=>'multipart/form-data']) !!}
            ...
            <div class="form-group">
                ...
                <select id="animal" name="animal" class="form-control" required>
                    ...
                    @foreach($animal as $row)
                        <option value="{{$row->id}}" {{($row->id == $info->animal_types) ? 'selected' : '' }}>{{$row->animal}}</option>
                    @endforeach
                </select>
            </div>
